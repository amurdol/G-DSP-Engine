...
def export_verilog_include(
    ...
    """
    Export as a Verilog include file (.vh / .v) with a localparam array.

    Output example:
        localparam signed [11:0] COEFF [0:32] = '{
            12'sh7FF,
            12'sh001,
            ...
        };
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        f.write(f"localparam signed [{w-1}:0] {array_name} [0:{n-1}] = '{{\n")
        ...
        for i, v in enumerate(data):
            ...
            f.write(
                f"    {w}'sh{tc:0{(w+3)//4}X}{comma}"
                f"  // [{i:3d}] = {int(v):+6d}"
                f"  ({fixed_to_float(v, fmt):+.6f})\n"
            )
